...
fn expand_xcdr_v2_type(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let encoded_len = field_tys.iter().fold(
        quote!(::up_wire_xcdrv2::XCDR_V2_ENCAPSULATION_LEN),
        |offset, ty| {
            quote!(::up_wire_xcdrv2::xcdr_v2_field_end(
                #offset,
                <#ty as ::up_wire_xcdrv2::XcdrV2Field>::ALIGNMENT,
                <#ty as ::up_wire_xcdrv2::XcdrV2Field>::ENCODED_LEN,
            ))
        },
    );
    ...
    Ok(quote! {
        impl #impl_generics ::up_wire_xcdrv2::XcdrV2Mappable
            for #name #ty_generics #where_clause
        {
            ...
            fn encode_xcdr_v2(
                ...
                let encoded_len =
                    <Self as ::up_wire_xcdrv2::XcdrV2Mappable>::ENCODED_LEN;
                let mut encoder = ::up_wire_xcdrv2::XcdrV2Encoder::new(dst, encoded_len)?;
                encoder.write_encapsulation()?;
                #(
                    ::up_wire_xcdrv2::XcdrV2Field::encode_field(
                        &self.#field_idents,
                        &mut encoder,
                    )?;
                )*
            ...
            fn decode_xcdr_v2(src: &[u8]) -> Result<Self, ::up_rust::UWireError> {
                let encoded_len =
                    <Self as ::up_wire_xcdrv2::XcdrV2Mappable>::ENCODED_LEN;
                let mut decoder = ::up_wire_xcdrv2::XcdrV2Decoder::new(src, encoded_len)?;
                decoder.read_encapsulation()?;
                let value = Self {
                    #(
                        #field_idents: <#field_tys as ::up_wire_xcdrv2::XcdrV2Field>::decode_field(
                            &mut decoder,
                        )?,
                    )*
                };
